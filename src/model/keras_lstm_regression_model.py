...
class LSTMRegressionModel:
    ...
    HISTORY_DEFAULT = 1 # how many days of features should we include in each feature vector
    ...
    def __init__(self, train_data, val_data=None, test_data=None):
        ...
        self.train_x = self.train_x.reshape((len(init_x), 1, self.num_features))
        ...
        self.history_size = LSTMRegressionModel.HISTORY_DEFAULT
    ...
    def create_model(self, num_features, num_blocks=64, seeded=False):
        used_seed = None
        if seeded:
            used_seed = self.SEED_DEFAULT
        model = Sequential()
        model.add(LSTM(num_blocks, return_sequences=True, batch_input_shape=(LSTMRegressionModel.BATCH_SIZE_DEFAULT, 1, self.num_features), stateful=True))
        model.add(Dropout(0.2, seed=used_seed))
        model.add(LSTM(num_blocks, return_sequences=True, stateful=True))
        model.add(Dropout(0.2, seed=used_seed))
    ...
    def train(self):
        if self._model is not None:
            ...
            for i in range(100):
                self._model.fit(self.train_x, self.train_y, epochs=1, batch_size=LSTMRegressionModel.BATCH_SIZE_DEFAULT, verbose=2, shuffle=False)
