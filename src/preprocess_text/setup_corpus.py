...
def setup_corpus(article_parser_type, article_dir, corpus_name, max_articles, per_date, use_big_data, min_length=5, corpus_dir="/opt/nlp_shared/corpora/WebhosePoliticalNewsCorpora"):
    ...
    for doc in myparser.yield_articles():
        ...
        print("Adding articles to corpus: {0}% complete ({1} of {2}).\r".format(i * 100.0 / max_articles, i, max_articles))
        sys.stdout.flush()
...
def setup_nyt_corpus():
    return setup_corpus(NYTArticleParser, "/opt/nlp_shared/data/news_articles/nytimes/nytimes_json/", "NYTCorpus", 1e8, True, True, corpus_dir="/opt/nlp_shared/corpora/NytCorpora")
